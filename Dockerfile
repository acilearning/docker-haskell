...
RUN \
  ...
  if test -n "$HLS_VERSION"; then \
    if echo "$HLS_VERSION" | grep --extended-regexp --quiet '^[0-9a-f]{40}$'; then \
      ghcup --verbose compile hls --cabal-update --ghc "$GHC_VERSION" --git-describe-version --git-ref "$HLS_VERSION" -- --constraint='ghc-exactprint > 1.6.0' --index-state='2022-11-11T21:44:45Z' --ghc-options='+RTS -M2G -RTS'; \
    else \
